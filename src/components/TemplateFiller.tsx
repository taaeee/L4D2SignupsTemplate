...
export default function TemplateFiller({ template }: { template: Template }) {
  ...
  const addFriendToRoster = (friend: Friend) => {
  ...
  const updatePlayer = (index: number, key: keyof Player, value: string | boolean) => {
    const newPlayers = [...players];
    (newPlayers[index] as any)[key] = value;
    setPlayers(newPlayers);
  };

  const updatePlayerCustomValue = (index: number, key: string, value: string) => {
    const newPlayers = [...players];
    newPlayers[index].custom[key] = value;
    setPlayers(newPlayers);
  };

  const toggleCustomRole = (index: number) => {
    const newPlayers = [...players];

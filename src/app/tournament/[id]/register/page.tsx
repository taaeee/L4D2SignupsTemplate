...
export default function RegisterTeam() {
  ...
  const fetchSteamLinked = async () => {
    try {
      ...
      if (accountData.accounts) {
        setHasSteamLinked(accountData.accounts.some((acc: any) => acc.provider === "steam"));
      }
    } catch (e) {
      console.error(e);
    }
  ...
  const selectFriend = (friend: Friend) => {
    if (activePlayerIndex === -1) {
      ...
      const newP = [...players];
      if (activePlayerIndex !== null && newP[activePlayerIndex]) {
  ...
  const handleRemovePlayer = (pId: number) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const membersToInsert = validatedPlayers.map((vp, idx) => ({
        ...
        name: vp.name,
        role: JSON.stringify({
          title: idx === 0 ? "Captain" : idx === 1 ? "Co-Captain" : "Member",
          answers: vp.answers || {}
        }),
      ...
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
        ...
        <div className="glass-panel" style={{ padding: "2rem" }}>
          ...
          <div className="flex-col gap-4">
            ...
            {tpl.fields?.filter((f: any) => f.name !== "Country" && f.name !== "Region" && f.name !== "Tag").map((field: any) => (
              <div key={field.name}>
            ...
            {players.map((player, index) => (
              <div key={player.id} style={{ padding: "1.5rem", background: "rgba(0,0,0,0.2)", borderRadius: "var(--radius-md)", border: "1px solid var(--border-light)", position: "relative" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1rem" }}>
                  ...
                  {tpl.playerFields?.map((field: any) => (
                    <div key={field.name}>

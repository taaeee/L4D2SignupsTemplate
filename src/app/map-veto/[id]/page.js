...
export default function MapVetoInterface() {
  ...
  const [vetoSession, setVetoSession] = useState(null);
  ...
  const checkAdminStatus = async (tournamentId) => {
    ...
    if (data) {
      ...
      const isMod = data.moderators?.includes(session?.user?.id);
  ...
  const fetchTeams = async (idA, idB) => {
    const { data } = await supabase
      .from("teams")
  ...
  const getNextTurnInfo = (format, history, maps, currentTurnId) => {
    const pickCount = history.filter(h => h.action === "pick").length;
  ...
  const handleMapAction = async (mapName) => {
    ...
    const map = vetoSession.state.maps.find(m => m.name === mapName);
    ...
    // Update map status
    const newMaps = vetoSession.state.maps.map(m => {
      if (m.name === mapName) {
    ...
    if (status === "completed" && availableCount === 1 && vetoSession.format !== "to2") {
      ...
      finalMaps = newMaps.map(m => m.status === "available" ? { ...m, status: "picked", isDecider: true } : m);
    }
  ...
  const handleEndVeto = async () => {
    const { error } = await supabase.from("map_vetoes").delete().eq("id", id);
    if (!error) {
  ...
  const filteredMaps = vetoSession.state.maps.filter(m => filter === "all" || m.type === filter);
    ...
    <div className="container" style={{ padding: "2rem", maxWidth: "1200px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(250px, 1fr))", gap: "1.5rem" }}>
        {filteredMaps.map(map => {
          ...
          const historyEntry = vetoSession.state.history.find(h => h.mapName === map.name);
          ...
          return (
            <div
              ...
              style={{
                ...
                transform: isVetoed ? "scale(0.95)" : "scale(1)",
                opacity: isVetoed ? 0.4 : 1,
                border: isPicked ? "3px solid var(--success)" : "3px solid transparent",
              ...
              onMouseEnter={(e) => {
                if (isMyTurn && isAvailable) {
                  ...
                  e.currentTarget.style.border = `3px solid ${expectedAction === "pick" ? "var(--success)" : "var(--danger)"}`;
                }
              }}
              onMouseLeave={(e) => {
                if (isMyTurn && isAvailable) {

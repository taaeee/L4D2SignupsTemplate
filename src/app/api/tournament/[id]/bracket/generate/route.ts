...
export async function POST(req: Request, { params }: { params: any }) {
  try {
    ...
    let insertMatches: any[] = [];
    
    if (isSwiss) {
      const swissRounds = body.swissRounds || Math.ceil(Math.log2(N));
      
      // En Swiss, generamos sólo la Ronda 1.
      for (let i = 0; i < N / 2; i++) {
    ...
    // Seeding Algorithm
    function getSeedOrder(p: number): number[] {
      if (p === 1) return [1];
    ...
    const validMatches = insertMatches.map((m: any) => ({
      id: m.id,
    ...
    if (isSwiss) {
      newTemplateJson.swissRounds = body.swissRounds || Math.ceil(Math.log2(N));
      newTemplateJson.currentSwissRound = 1;
    }
